...
class JustVerClick(nextcord.ui.View):

    def __init__(
            self,
            callback: Callable,
            user_to_click: nextcord.Member,
            *args,
            timeout: int = 30,
            **kwargs
    ):
        self.callback = callback
        self.utc = user_to_click
        self.args = args
        self.kwargs = kwargs
        super(JustVerClick, self).__init__(timeout=timeout)

    @nextcord.ui.button(label="Начать", custom_id=f"{secrets.token_hex(10)}")
    async def verify(self, _, inter):
        if (
                self.utc == inter.user
        ):
            await self.callback(*self.args, **self.kwargs, inter=inter,
                                button=_, )
            self.stop()
        else:
            message = ResponseEmbed(
                user=inter.user,
                embed=nextcord.Embed(
                    title="Не вашего ума дело",
                    description="!!!"
                ),

            ).error

            await inter.send(embed=message, ephemeral=True)
...
class GuildView(DataMixin, nextcord.ui.View):
    ...
    async def __check(self, inter: Interaction, _btn: nextcord.Button) -> \
            tuple[bool, str, any]:
        c = CheckUser(
            bot=self.bot,
        ...
        if hi:
            res = False
            msg = "Вы уже отправили заявку, сейчас нужно дождатся пока вас " \
                  "примут в гильдию.\n\n**Если вас не примут - вы сможете " \
                  "отправить заявку снова**"

        elif ii:
            res = False
            msg = "Вы уже начали создавать заявку, если это не так, " \
                  "обратитесь " \
                  "" \
                  "" \
                  "к <@!686207718822117463>."

        elif im:
            res = False
            msg = "Вы являеетесь гильд-мастером, вы не можете сменить " \
                  "гильдию, " \
                  "" \
                  "" \
                  "пока не закончится ваш срок правления."

        elif iig:
            res = False
            msg = "Вы уже находитесь в гильдии, попросите вашего мастера " \
    ...
    async def __callback(self, inter: Interaction, btn: nextcord.Button):
        try:
            ...
            print("Not checked", not checked)

            if not checked:
                print("Error callback")
                return await self.bad_callback(interaction=inter,
                                               message=message)
            ...
            async def callback(inter: nextcord.Interaction, button):
                manager.creating_invite(True)

                modal = InviteModal.InviteModal(
                    mobile=False,
                    bot=self.bot,
                    guild=guild,
                )
                ...
                await inter.delete_original_message(delay=1)
            ...
            view = JustVerClick(
                timeout=30,
                callback=callback,
                user_to_click=inter.user
            )

            embed = ResponseEmbed(
                embed=nextcord.Embed(
                    title="Можно начать!",
                    description="Нажмите на кнопку ниже, и начните заполнять "
                                "заявку!"
                                "\n\n"
                                "**P.S.** ||Она исчезнет||, "
                                "через **3*0*** секунд!"
    ...
    @nextcord.ui.button(label="Дендро", style=nextcord.ButtonStyle.blurple,
                        custom_id="Dendro")
    async def dendro(
    ...
    @nextcord.ui.button(label="Гидро", style=nextcord.ButtonStyle.blurple,
                        custom_id="Hydro")
    async def hydro(
            self, button: nextcord.Button, interaction: nextcord.Interaction
    ):
    ...
    @nextcord.ui.button(label="Пиро", style=nextcord.ButtonStyle.blurple,
                        custom_id="Pyro")
    async def pyro(
            self, button: nextcord.Button, interaction: nextcord.Interaction
    ):
    ...
    @nextcord.ui.button(label="Крио", style=nextcord.ButtonStyle.blurple,
                        custom_id="Cryo")
    async def cryo(
            self, button: nextcord.Button, interaction: nextcord.Interaction
    ):
    ...
    @nextcord.ui.button(label="Анемо", style=nextcord.ButtonStyle.blurple,
                        custom_id="Anemo")
    async def anemo(
            self, button: nextcord.Button, interaction: nextcord.Interaction
    ):
    ...
    @nextcord.ui.button(label="Электро", style=nextcord.ButtonStyle.blurple,
                        custom_id="Electro")
    async def electro(
            self, button: nextcord.Button, interaction: nextcord.Interaction
    ):
    ...
    @nextcord.ui.button(label="Гео", style=nextcord.ButtonStyle.blurple,
                        custom_id="Geo")
    async def geo(
            self, button: nextcord.Button, interaction: nextcord.Interaction
    ):
    ...
    @nextcord.ui.button(label="Случайная гильдия",
                        style=nextcord.ButtonStyle.green, custom_id="random")
    async def random(
            self, button: nextcord.Button, interaction: nextcord.Interaction
    ):
        await self.__callback(interaction, button)
...
class InviteView(DataMixin, nextcord.ui.View):
    ...
    async def __check(self, inter: Interaction) -> Tuple[bool, str]:
        gm_role_id = GuildsManager(self.guild.lower()).get_data(
...
class YesCloseVIew(nextcord.ui.View):
    ...
    def __init__(
            self, wait_for: int, users_tc: nextcord.role, modal: callable,
            **kwargs
    ):
...
#
# 	def __init__(self, guild: str, speech: callable, topic: callable, message:
# 	nextcord.Message):
...
#
#
# 	@nextcord.ui.button(label="Изменить агитационную речь",
...
# 			self.stop()
#
# 	@nextcord.ui.button(label="Изменить тему канала",
...
#
# 	@nextcord.ui.button(label="Выбрать победителя!",
# 	style=nextcord.ButtonStyle.green)
...
class ListEmbedsView(nextcord.ui.View):
    def __init__(
            self, embeds: List[nextcord.Embed], user: nextcord.Member,
            base_embed: nextcord.Embed, button: bool = False,
            edit: callable = None
    ):
    ...
    async def show(self, _btn, _inter: Interaction):
        ...
        try:
            self.embed: nextcord.Embed or None = self.menu.data.get('embed',
                                                                    self.embed)
...
class CallbackCloseView(nextcord.ui.View, DataMixin):
    ...
    def __init__(
            self, callback: callable, user: nextcord.User, is_async: bool,
            *args, **kwargs
    ):
