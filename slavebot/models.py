...
#
#
# def embed_stan(embed: nextcord.Embed = nextcord.Embed(title='Error EmptyEmbed 404', color=nextcord.Color.red())):
# 	offset()
#
# 	embed.set_author(name=f"Система управления гильдями!", icon_url="https://cdn.discordapp.com/attachments/862044149779529750/956595788979535902/256.png")
#
...
#
# 	israel_tz = datetime.timezone(offset=datetime.timedelta(hours=int(offset_)), name="UTC")
...
# 	:return: Перебирает роли User и возвращает значение в зависимости от того, какие роли у него есть:
# 	1) NIG - не в гильдии
# 	2) IGM - Гильд-мастер
# 	3) IGV - Смотритель за гильдиями
# 	4) [List] - участник одной из гильдий, в списке возвращается роль гильдии + её гильд-мастер
# 	"""
# 	try:
# 		try:
# 			guild_master_roles = [user.guild.get_role(role) for role in get_tags_by_type("TIME_ROLE", "guild-master")]
# 			guild_vision_roles = [user.guild.get_role(role) for role in get_tags_by_type("TIME_ROLE", "guild-vision")]
# 			guild_member_roles = [user.guild.get_role(role) for role in get_tags_by_type("ROLE", "role")]
...
#
# 					if len(list_for_check) >= 1:
# 						gm = NEDOGUILD().get_one_tag(guild=list_for_check[0].name, tag="guild_president")
...
# 		super().__init__()
# 		self.msg = "Запись с этим ID уже находится в базе данных!" if msg is None else msg
#
# 	def __repr__(self):
...
# 	def __init__(self, msg=None):
# 		super().__init__()
# 		self.msg = "Записи с этим ID нет в базе данных!" if msg is None else msg
#
# 	def __repr__(self):
...
# class InviteSend:
#
# 	def __init__(self, guild: str, interaction: nextcord.Interaction or nextcord.User, bot: commands.Bot, params: list, invite_data):
#
...
# 		self.user = interaction.user if isinstance(interaction, nextcord.Interaction) else interaction
# 		self.guild = guild
# 		self.params = params if isinstance(params, list) and len(params) >= 3 else print()
#
# 		self.invite = {
...
#
# 			gm_role = NEDOGUILD().get_one_tag(guild=self.guild, tag="guild_master_role")
...
# 			def time(secs: int):
# 				try:
# 					timesta = datetime.datetime.now() + datetime.timedelta(seconds=secs)  # 259200 -> Три дня
#
# 					timestamp = str(timesta.timestamp()).split(".")[0]
# 					timestamp = f"<t:{timestamp}:R>"
# 					return timestamp
# 				except Exception as exception:
# 					LOGGER.error(f"{self.__class__.__name__} -> send -> timestamp exception {exception}")
# 					raise LookupError(exception) from exception
#
# 			a: list = self.params
#
# 			embed = nextcord.Embed(title=f"Заявка для {self.guild}.",
# 			                       description=f"{self.user.mention} возжелал вступить в гильдию {self.guild}, вот что он написал в заявку:\n\n\n"
# 			                                   f"Возраст: {a[0]}\n\n"
# 			                                   f"Почему ГМ {self.guild} должен принять его: {a[1]}\n\n"
# 			                                   f"Любимые персонажи: {a[2]}\n\n"
# 			                                   f"Пол: {a[3]}\n\n"
# 			                                   f"Активность в: {a[4]}\n\n"
# 			                                   f"Что он хочет от {self.guild}: {a[5]}\n\n\n\n"
# 			                                   f"**Срок действия заявки истечёт:** {time(259200)}!",
# 			                       colour=nextcord.Color.from_rgb(80, 141, 234))
...
#
# 				await self.user.add_roles(role, reason="По приказу генерала гафса!")
#
# 				#
# 				# ---- message for user ----
# 				#
...
# 					description=f"Ваша заявка была рассмотренна и ГМ вынес решение о принятии вас в {self.guild}.✔️",
...
# 				x = GuildDefense(self.bot, server=guild, user=self.user.id, guild=self.guild)
# 				x.set_cooldown(30.0 * 60.0)
# 				embed_acc = nextcord.Embed(
# 					title="Сожалею!",
# 					description=f"Вы не смогли вступить в гильдию {self.guild}!\n\n\nВступить в гильдию снова можно будет {x.dc__timestamp}.",
#
...
#
# 		except Exception as exc:
# 			LOGGER.error(f"{self.__class__.__name__} -> send exception" + str(exc))
# 			raise LookupError(exc) from exc
# 		try:
# 			self.idta.delete()
...
#
# 		# get
# 		# watchers = [print(watcher_id) and server.get_member(watcher_id) for watcher_id in watchers_id if len(str(watcher_id)) == 18 and isinstance(watcher_id, int)]
# 		# async fetch
#
# 		#
...
# 			description=f"Смотрители уже уже знают об этом и работаеют над этим(читают морали ГМ-у {self.guild})",
...
# 			usr += f"<@&{watcher_id}>"
#
# 		embed_edit = embed_stan(nextcord.Embed(title=f"Проигнорированная заявка в {self.guild}...", description=f"{self.user.mention}"))
# 		try:
# 			await msg.edit(content=f"{usr}", embed=embed_edit, view=NonBts())
# 		except Exception as exc:
# 			LOGGER.error("Cannot edit ignored invite message because " + str(exc))
#
#
...
#
# 	def __repr__(self):
# 		return f"Объект заявки в базе данных юзера (\\{self.invite['user_id']}\\)"
#
# 	def __str__(self):
# 		return f"Объект заявки в базе данных юзера (\\{self.invite['user_id']}\\)"
#
...
# 				invites_json["invites_numbers"] = invites_json["invites_numbers"] + 1
#
...
# 		except Exception as exc:
# 			LOGGER.error(self.__class__.__name__ + " -> save exception " + str(exc))
# 			raise LookupError(exc) from exc
#
# 	@property
# 	def inv(self):
...
# class InviteParams:
#
# 	def __init__(self, bot: commands.Bot, interaction: nextcord.Interaction, button):
#
# 		self.bot = bot
...
# 				"in_creating_invite": status if isinstance(status, bool) else True
# 			}
#
...
#
# 			try:
# 				return invites_dict[f"invite_{self.interaction.user.id}"] if isinstance(invites_dict[f"invite_{self.interaction.user.id}"], bool) else True
# 			except Exception as exc:
...
# 					return False
# 			except Exception as exc:
# 				LOGGER.error(f"Error in InviteParams -> check_invite_status: {exc}")
# 				return False
#
# 		def del_status():
# 			with open("additional_files/invites.json", "r", encoding="UTF-8") as invites_file:
...
# 				json.dump(invites_dict, invites_file_w, indent=4)
#
# 		async def something_went_wrong(whats_wrong: str, exception: Exception = None):
# 			"""
# 			:param: whats_wrong: String object, with text about the exception
# 			"""
# 			try:
# 				await self.interaction.response.send_message(f"Что-то пошло не так, мы пытаемся выяснить что именно. \nПреждевременный анализ выявил, что причиной стал: {whats_wrong}",
# 				                                             ephemeral=True)
# 				LOGGER.error(f"Автоматическое расспространение ошибки: Exception Type: {type(exception)}\n\n Exception Desc: {exception}") if exception is not None else None
# 			except Exception as exc:
# 				raise LookupError() from exc
#
# 		try:
# 			async def create_channel(interaction: nextcord.Interaction) -> nextcord.TextChannel:
#
# 				try:
...
# 					overwrites = {
# 						guild.default_role: nextcord.PermissionOverwrite(view_channel=False, send_messages=False),
# 						self.interaction.user: nextcord.PermissionOverwrite(view_channel=True, send_messages=True),
# 						yat: nextcord.PermissionOverwrite(view_channel=True, send_messages=True),
# 						awa: nextcord.PermissionOverwrite(view_channel=True, send_messages=True),
# 						self.bot.user: nextcord.PermissionOverwrite(view_channel=True, send_messages=True)
# 					}
#
# 					channel = await guild.create_text_channel(
# 						position=0,
# 						category=category,
# 						name=f"Канал для создания заявки {self.interaction.user.name}",
# 						overwrites=overwrites,
# 						reason="По приказу генерала гафса",
# 						topic=f"Автоматически созданный канал для заполнения заявки пользователя под ником: {self.interaction.user.display_name}",
# 						slowmode_delay=1
#
# 					)
#
...
# 				await chat.send(embed=embed_stan(embed=nextcord.Embed(title="Заполнение заявки было отменено, удаляю канал...", color=nextcord.Color.red())))
# 				await asyncio.sleep(5)
# 				await chat.delete()
#
# 				await self.interaction.user.send(embed=embed_stan(embed=nextcord.Embed(title="Вы отменили заполнение заявки.", color=nextcord.Color.red())))
#
# 			defense = GuildDefense(self.bot, self.interaction.user.guild, self.interaction.user.id, self.button.custom_id if not None else self.button.label)
#
# 			if defense.is_on_cooldown[0]:
# 				props = defense.is_on_cooldown[1]
# 				time_until_2m = datetime.datetime.fromtimestamp(float(props.get("until", None))) + datetime.timedelta(seconds=120)
#
# 				timestamp_until = str(time_until_2m.timestamp()).split(".")[0]
#
# 				return await self.interaction.response.send_message(
# 					ephemeral=True,
# 					content=f"Недавно вы вышли из {props.get('guild', '(ошибка)')}, вы снова сможете вступить в гильдию <t:{timestamp_until}:R>."
# 				)
#
# 			if check_invite_status() is True:
# 				return await self.interaction.response.send_message(ephemeral=True,
...
# 			await something_went_wrong(whats_wrong="Недостаточно прав для создания канала", exception=excf)
# 		except nextcord.HTTPException as exch:
# 			await something_went_wrong(whats_wrong="Ошибка создания канала", exception=exch)
# 		except nextcord.InvalidArgument as exci:
# 			await something_went_wrong(whats_wrong="Неверно указаны параметры создания канала", exception=exci)
# 		except Exception as e:
# 			await something_went_wrong(whats_wrong="Ошибка создания канала", exception=e)
# 		else:
# 			try:
# 				await chat.send(self.interaction.user.mention)
#
# 				async def get_msg(wait: int, wait_for: nextcord.Embed, message: nextcord.Message = None) -> nextcord.Message or None:
# 					def check(m: nextcord.Message):
# 						# NEDOGUILD vision role user_id's
# 						counter = 0
# 						if m.channel == chat:
# 							g_v_r_i_s: list = get_tags_by_type("TIME_ROLE", "guild-vision")
# 							member: nextcord.Member = self.interaction.guild.get_member(m.author.id)
# 							author_roles = [role.id for role in member.roles]
#
# 							if member == self.interaction.user and m.channel == chat:
# 								result = True
# 							else:
# 								for rl in g_v_r_i_s:
...
# 										counter += 1
#
# 								if counter >= 1 and str(m.content) == "отменить-заполнение":
# 									result = True
# 								elif counter >= 1 and str(m.content).startswith("-=-"):
# 									result = True
# 								else:
# 									result = False
# 						else:
...
#
# 					try:
# 						await chat.send(embed=wait_for) if message is None else await message.edit(embed=wait_for)
# 					except Exception as exc:
# 						LOGGER.error(f"get_msg error \n {exc}")
# 						raise LookupError() from exc
# 					try:
#
# 						wait_for_msg = await self.bot.wait_for("message", check=check, timeout=wait)
#
...
# 						return wait_for_msg
#
# 				embed_start = embed_stan(embed=nextcord.Embed(title="Сборщик данных <<Український хакер віталя>>",
# 				                                              description=f"Для вступления в гильдию "
# 				                                                          f"{self.button.custom_id if self.button.custom_id else self.button.label} "
...
# 				                                                          f"вы передумали - просто не отвечайте на это сообщение 30 секунд.",
...
#
# 					embed_params = embed_stan(nextcord.Embed(title="Отлично!", description="Теперь начнём заполнение заявки. \n\n\n**Собираю данные...**", color=nextcord.Color.green()))
...
# 					params = ["ваш возраст, просто цифры.\n\nP.S. Опционально, можете написать белибирду типа: **sgjhblkzdrgb**. \nно учитывайте, что некоторые ГМ-ы очень сильно заостряют на этом "
# 					          "внимание.\n",
# 					          # --------------------------------------------------------------------------------------------------------
# 					          f"причина по которой мастеру {self.button.custom_id if self.button.custom_id else self.button.label} будет интересно принять вас.\n\n"
# 					          f"P.S. Это основная часть заявки, мастер будет определять брать вас или нет в основном из этого параметра.",
# 					          # --------------------------------------------------------------------------------------------------------
# 					          "любимого/ых героя/ев из Genshin Impact.",
# 					          # --------------------------------------------------------------------------------------------------------
# 					          "Какого вы пола.",
# 					          # --------------------------------------------------------------------------------------------------------
# 					          "Где вы больше всего активны? (в текстовом чате или в войсе(голосовой чат))",
# 					          # --------------------------------------------------------------------------------------------------------
# 					          f"Что вы хотели бы от {self.button.custom_id if self.button.custom_id else self.button.label} если вас примут (ивенты, постоянный актив, помощь)"
#
# 					          ]
#
# 					params_ac = []
...
# 						# УТРО ОБНОВИТЬ БОТА!!!
#
# 						embed_param = embed_stan(embed=nextcord.Embed(title=f"Сборщик данных <<Український хакер віталя>>, вопрос №{question}",
...
# 						p = await get_msg(wait=time_wait * 60, wait_for=embed_param, message=mess)
...
# 					await chat.trigger_typing()
#
# 					embed_cool = embed_stan(embed=nextcord.Embed(title="Сборщик данных <<Український хакер віталя>>", description=f"Сбор данных завершён, заявка отправлена, она будет расмотренна в "
# 					                                                                                                              f"течении 72 часов. \n\n\nДанный чат будет удален через 5-4-3-2...",
# 					                                             color=nextcord.Color.green()))
# 					if params_ac[0] and params_ac[1] and params_ac[2] == "Не заполненное поле.":
#
# 						embed_bad = embed_stan(embed=nextcord.Embed(title="Ваша заявка не заполнена", description="Вы не ответили ни на один из вопросов, ваша заявка не будет отправлена никуда.",
# 						                                            color=nextcord.Color.red()))
#
# 						await chat.trigger_typing()
# 						await asyncio.sleep(2)
...
#
# 			except Exception as exc:
# 				LOGGER.critical(f"Ошибка в сборе параметров для юзера {self.interaction.user}\n\n{exc}")
#
# 				await chat.trigger_typing()
# 				await chat.send(embed=embed_stan(embed=nextcord.Embed(title="ERROR", description="У нас произошла ошибка, данный канал будет удален через несколько секунд. \n\n\nСведения об ошибке: "
...
#
# 	for x in type_:
# 		embed.add_field(name=f"**{x}**", value=f"`{type_[x]}`", inline=inline(f))
# 		f += 1
# 		if f == 1:
# 			pass
#
...
#
# 		self.args = [self.guild, self.agi, self.users, self.guild_master, self.guild_chat_desc, self.channel, self.role, self.guild_master_role]
#
...
# 			self.guild_master_role: kwargs.get("guild_master_role", 000000000000000000)
#
# 		}
#
